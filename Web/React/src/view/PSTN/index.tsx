...
function PSTN({
  ...
  const checkMobile = (_: unknown, value: string) => {
    ...
    if (!_value) {
      return Promise.reject(new Error(t('pstn.phone.required')));
    }
    if (/^1[3-9]\d{9}$/.test(_value)) {
      return Promise.resolve();
    }
    return Promise.reject(new Error(t('pstn.phone.error')));
  };
  ...
  return (
    <div className='stage-wrapper'>
      <div className='stage'>
        ...
        <Header onExit={onExit} instanceId={result?.instanceId} reqId={result?.reqId || error?.reqId} />
            ...
            <>
              ...
              >
                <Form.Item
                  name='phone'
                  className='_phone'
                  label={t('pstn.phone.label')}
                  childElementPosition='right'
                  rules={[{ validator: checkMobile }]}
                >
                  <Input type='number' onChange={console.log} placeholder={t('pstn.phone.placeholder')} />
                </Form.Item>
                <div className='_tip'>{t('pstn.phone.tip')}</div>
                <Form.Item
                  label={t('pstn.interrupt.label')}
                  name='interrupt'
                  childElementPosition='right'
