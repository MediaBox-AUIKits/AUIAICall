//
//  AUIAICallRobotStateAnimation.swift
//  AUIAICall
//
...
@objcMembers open class AUIAICallRobotStateAnimation: UIView {
    ...
    open func updateRobotState(newState: ARTCAICallEngine.RobotState) {
        if self.robotState == newState {
            return
        }
        
        debugPrint("AUIAICallRobotStateAnimation: updateRobotState:\(newState)")
        let oldState = self.robotState
        self.robotState = newState
        self.listeningAniView.isHidden = self.robotState != .Listening
        self.thinkingAniView.isHidden = self.robotState != .Thinking
        self.speakingAniView.isHidden = self.robotState != .Speaking
    ...
    open private(set) var robotState: ARTCAICallEngine.RobotState = .Listening
    ...
    open func start(_ pre: Bool = false) {
        ...
        debugPrint("AUIAICallRobotStateAnimation: start ani")
        self.isAni = true
    ...
    open func stop() {
        debugPrint("AUIAICallRobotStateAnimation: stop ani")
        self.isAni = false
...
@objcMembers open class AUIAICallListeningAniView: UIView {
    ...
    private lazy var listeningImg: UIImageView = {
        ...
        view.image = AUIAICallBundle.getCommonImage("ic_robot_state_listening")
        return view
    }()
