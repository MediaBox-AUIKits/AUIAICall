...
- 配置智能体Id及所在的区域，打开智能体场景配置文件`/Resources/AUIAIMain.bundle/agent_scenes.json`，根据智能体类型找到agent_id填写到对应的字段中，同样方式填写region配置项。例如：配置语音智能体时，找到agent_type为`VoiceAgent`的Scenes节点，将agent_id与region的字段值。
```json
// agent_scenes.json
    {
...
target '你的App target' do
    ...
    # AI实时互动通话场景SDK
    # 如果你的业务还需要支持消息对话，则使用“ARTCAICallKit/Chatbot”进行集成，把下面一行改为：pod 'ARTCAICallKit/Chatbot', '~> 2.11.0'
